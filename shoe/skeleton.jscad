...
function main(params) {
  ...
  var pl = p[p.length - 1];
  p = p.concat(curve([pl[0] - 0.5, pl[1] + params.chr], params.chr, 2.95 * TAU / 4, 0.6 * TAU / 4, 0.5),
  ...
  p = p.concat(curve([0, (params.chr + 1) / 2], (bs / 2) + params.chr + 1, 1.1 * TAU / 4, 2.9 * TAU / 4, 1),
