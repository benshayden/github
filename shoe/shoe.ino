...
#include "keycodes.h"
#define NEITHER_MODE 0
#define MODE_0 1
#define MODE_1 2
#define BOTH_MODES 3
const uint16_t keymap_keys[4][] PROGMEM = {
#include "keymap"
};
...
bool toggle_modifier(uint8_t k) {
  switch (k) {
...
void loop() {
  ...
  uint16_t k = current_button + (mode * num_buttons);
  if (k < ARRAYSIZE(keymap_keys)) {
    k = pgm_read_word(keymap_keys + k);
  } else {
    k = 0;
  }
